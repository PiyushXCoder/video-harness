...
2. **Check the loudness of the two parts against each other.** A jump at the
...
3. **Join with the concat FILTER, not the concat demuxer.**
...
4. **Build the master subtitles.** The offset comes from the hook manifest, never
   from a typed number:
...
5. **Verify the join, and look at the seam.**
   ...
   Then extract frames either side of the seam and **look at them** — the hook's
   last frame and the body's first frame land back to back, and a hard cut
   between two unrelated shots is the one thing a viewer notices at 0:30:
...
6. **Spot-check the subtitles against the picture.** Pick a cue from late in the
...
7. **Report** the total runtime, the hook/body split, where `final.mp4` and
...
- **Don't scale or pad in the join.** If the parts don't match, the fix is
  upstream; every source in this project is already 2048×1280.
